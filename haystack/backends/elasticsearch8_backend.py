...
try:
    ...
    raise MissingDependency(
        "The 'elasticsearch8' backend requires the \
                            installation of 'elasticsearch>=8.0.0,<9.0.0'. \
                            Please refer to the documentation."
    )


class Elasticsearch8SearchBackend(ElasticsearchSearchBackend):
    ...
    def _get_doc_type_option(self):
        # ES8 does not support a doc_type option
        return {}
    ...
    def clear(self, models=None, commit=True):
        ...
        try:
            if models is None:
                self.conn.indices.delete(index=self.index_name, ignore_unavailable=True)
                ...
                for doc in generator:
                    actions.append({"_op_type": "delete", "_id": doc["_id"], "_index": self.index_name})
    ...
    def build_search_kwargs(
        ...
        **extra_kwargs
        ...
        if fields:
            ...
            kwargs["_source"] = fields  # Changed from stored_fields to _source
        ...
        if date_facets is not None:
            ...
            for facet_fieldname, value in date_facets.items():
                # ES8 uses calendar_interval or fixed_interval instead of interval
                interval = value.get("gap_by").lower()
                
                # Determine if we should use calendar_interval or fixed_interval
                calendar_intervals = ["minute", "hour", "day", "week", "month", "quarter", "year"]
                interval_key = "calendar_interval" if interval in calendar_intervals else "fixed_interval"
                ...
                kwargs["aggs"][facet_fieldname] = {
                    ...
                    "date_histogram": {"field": facet_fieldname, interval_key: interval},
                    "aggs": {
                        facet_fieldname: {
                            "date_range": {
        ...
        **kwargs
        ...
        try:
            ...
            raw_results = self.conn.search(
                body=mlt_query, index=self.index_name, source=True, **params
            )
        except elasticsearch.TransportError:
            if not self.silently_fail:
                raise
    ...
    def _process_hits(self, raw_results):
        # ES8 changed the hits.total structure
        hits = raw_results.get("hits", {})
        total = hits.get("total", 0)
        
        # In ES8, total can be an integer or an object with value
...
class Elasticsearch8SearchQuery(ElasticsearchSearchQuery):
